...
export default function AIChat() {
  ...
  const ask = async () => {
    ...
    while (reader) { const { done, value } = await reader.read(); if (done) break; setOut(p => p + dec.decode(value)) }
  }
  ...
  const Box = (
    <div className="rounded-2xl border p-4 shadow-xl bg-white/90 dark:bg-zinc-900/90 backdrop-blur">
      ...
      <textarea className="w-full border rounded p-2 mb-2 bg-transparent" rows={3}
        value={q} onChange={(e)=>setQ(e.target.value)} placeholder="Ask about my projects, skills, or experience…" />
  ...
  // Docked (inline under the Playground title)
  const Docked = slot && createPortal(
    <AnimatePresence mode="wait">
      {dock && (
        <motion.div key="docked" initial="out" animate="in" exit="out" variants={variants}
  ...
  const Floating = (
    <AnimatePresence mode="wait">
      {!dock && (
        <motion.div key="floating" initial="out" animate="in" exit="out" variants={variants}
